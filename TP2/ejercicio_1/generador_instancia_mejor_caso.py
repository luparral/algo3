...
def parAleatorioCiudades():'Senda', 'Hiros', 'Saita', 'Kawas', 'Fukuo', 'Kioto', 'Kumag', 'Sappo', 'Nagoy', 'Osaka', 'Yokoh', 'Tokio', 'Chiyo', 'Shibu', 'Taito', 'Kodai', 'Higas', 'Kushi', 'Yamag', 'Nishi', 'Itami', 'Matsu', 'Hitac', 'Bunky', 'Suzuk', 'Araka', 'Odawa', 'Numaz', 'Totto', 'Kishi', 'Isesa'
...
def parNoAleatorioDeCiudades(ciudadesSinA):
	ciudad_a = ciudadesSinA[random.randint(0,int(cantCiudades)-2)]
	ciudad_b = ciudades[random.randint(0,int(cantCiudades)-1)]
	
	while(ciudad_a == ciudad_b):
		ciudad_b = ciudadesSinA[random.randint(0,int(cantCiudades)-2)]
...
f = open('instancia_mejorCaso.txt', 'w')
f.write(ciudad_a+' '+ciudad_b+' '+str(n)+'\n')


ciudadesSinA = [ciudad for ciudad in ciudades if not ciudad == ciudad_a]

for i in range(0,int(n)):
	ciudad1,ciudad2 = parNoAleatorioDeCiudades(ciudadesSinA)
