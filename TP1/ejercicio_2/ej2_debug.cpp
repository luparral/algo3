...
//horizontes es un vector que tiene muchos horizontes adentro
//cada horizonte es un vector de coordenadas
//y una coordenada es una tupla de pares de ejes X e Y
...
int main(){
	...
	while(cant_edificios != 0){
		...
		for (int i = 0; i < cant_edificios; ++i){
			cout << "Ingrese edificio " << i << " coordenada x1: ";
			cin >> edificios[i][0]; //x1
			cout << "Ingrese edificio " << i << " coordenada y: ";
			cin >> edificios[i][1]; //y
			cout << "Ingrese edificio " << i << " coordenada x2: ";
		...
		//print para ver si todo esta bien
		for (int i = 0; i < cant_edificios; ++i){
			cout << "Coordenadas edificio " << i << ": (" << horizontes[i][0].first << ", " << horizontes[i][0].second << ") (" << horizontes[i][1].first << ", " << horizontes[i][1].second << ")" << endl;
		}

		Horizonte resultado = UnirHorizontes(horizontes, 0, horizontes.size()-1);

		cout << "\nResultado del contorno: " << endl;
...
//tengo que ver como hacer con las referencias no const (Preguntar a Lebron)
Horizonte Unir(const Horizonte& h1, const Horizonte& h2){
	//Inicializo un horizonte resultado
	//inicializo variables para mantener la ultima coordenada accedida de cada horizonte
	...
	//INVARIANTE: hasta la posicion i+j-1 del resultado, este contiene contorno formado hasta ese momento!!
	while( i < h1.size() || j < h2.size() ){
		...
		//en el caso de que no se termino de recorrer ningun horizonte aun
		//caso h1.x < h2.x	
