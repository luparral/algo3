...
int main(int argc, char** argv){
    ...
    printParticionDeSolucion(solucion, k);

    cout << "El peso de la solucion original es: " << endl << pesoDeSolucion(solucion, k, grafo) << endl;

    busqueda_local_2(solucion, k, grafo);

    cout << "La solucion modificada es: " << endl;
    for(int i = 0; i < solucion.size(); i++)
        cout << solucion[i] << ' ';

    cout << endl << "El peso de la solucion modificada es: " << endl << pesoDeSolucion(solucion, k, grafo) << endl;
