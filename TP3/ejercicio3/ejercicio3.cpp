...
//TODO: Estandarizar camelcase o underscore.
//cambiar los int de los pesos por float DONE
//estandarizar ingles o castellano
//refactorizar la función en main
...
int main (){
	...
	//pongo todos los vertices en la primer particion
	for(int nodo = 0; nodo < cant_nodos; nodo++){
		particiones[0].insert(nodo);
	}
	...
	for(int nodo = 0; nodo < cant_nodos; nodo++) {
		pesos.push_back(NodoConPeso(nodo, peso_intraparticion(nodo, g, particiones[0])));
	}
	...
	for(int nodo = 0; nodo < pesos.size(); nodo++){
		...
		float menorPeso = peso_intraparticion(pesos[nodo].nodo, g, particiones[0]);
		int aux = 0;
		...
		particiones[0].erase(pesos[nodo].nodo);
