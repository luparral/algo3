...
Peso resolver(int n, Pesos& pesos, int k, Sol& solucion, Grafo& grafo);
Peso pesoDeParticion(Part& particion, Pesos& pesos);
...
void getPartitions(Pesos& pesos, Peso optimo_peso, int n, int k, vector<Part>& itetarivePartitions);
...
int main(){
	...
	while(n != 0){
		...
		averagesFile << "Grafo:" << endl << n << " " << m << " " << k << endl;
		...
		int cantRepeticiones = 10;
		while(cantRepeticiones > 0){
			// cout << cantRepeticiones;
			cout << endl;
			...
			Sol solucion;
			solucion.reserve(n);
			Peso pesoDeResolucion = 0;
			if (k < n)
				pesoDeResolucion = resolver(n, pesos_ij, k, solucion, g);
			else
			...
			if(cantRepeticiones == 1){
				...
				averagesFile << "Respuesta: ";
				for(int i = 0; i < n; i++)	averagesFile << solucion[i] << " ";
				averagesFile << endl;
				averagesFile << "Peso: ";
				averagesFile << pesoDeResolucion << endl;
			}
		...
		if(averagesFile.is_open())
	    	averagesFile << "Mejor tiempo para "<< n <<" nodos, " << m << " aristas y k=" << k << " : " << 1000.0 * menorTiempo << " milisegundos" << endl << endl << endl << endl;
...
Peso resolver(int n, Pesos& pesos, int k, Sol& solucion, Grafo& grafo){
	...
	Part resultado = generarParticionInicial(n,pesos,k,grafo);
	Peso optimo_peso = pesoDeParticion(resultado,pesos);

	getPartitions(pesos, optimo_peso, n, k, itetarivePartitions);
	
	for (int i = 0; i < itetarivePartitions.size(); ++i){
		Peso pesoParticion = pesoDeParticion(itetarivePartitions[i],pesos);
		if(pesoParticion < optimo_peso){
			resultado = itetarivePartitions[i];
			optimo_peso = pesoParticion;
		}
	}
	Peso pesoDelResultado = pesoDeParticion(resultado,pesos);
	...
	for (int i = 2; i <= n; ++i){
		...
		for (int j = 0; j < itetarivePartitionsCant; ++j){
			...
			for (int a = 0; a < actualPart.size(); ++a){
				...
				int pesoParticion = pesoDeParticion(actualPart,pesos);
				if(pesoParticion < optimo_peso){
